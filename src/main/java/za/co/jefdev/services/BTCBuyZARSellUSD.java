...
public class BTCBuyZARSellUSD extends BaseCalc {
    ...
    @Override
    public String calcProfit(Double dollarAmt){
        Double numOfBTC = (dollarAmt*zarUsd)/lunoBTCBid;
        Double btcAfterTransfer = numOfBTC- BaseCalc.LUNO_BTC_TRANSFER_FEE;
        Double sellAmount = btcAfterTransfer * cexUSDBTCSellPriceZar;
        Double profit = sellAmount - (dollarAmt * zarUsd);

        return "Buying for: $ " + BaseCalc.formatter.format(dollarAmt) + "\n" +
